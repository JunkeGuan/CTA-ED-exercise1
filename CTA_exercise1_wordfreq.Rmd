---
title: "CTA_exercise1_wordfreq - with answers"
author: "Junke Guan"
date: "5/02/2026"
...
```{r}
tidy_des$raceword <- as.integer(
  grepl(
    "race|racial|ethnic|ethnicity|migration|migrant|immigrant|refugee",
    tidy_des$word
  )
)
```
...
```{r}
race_counts <- tidy_des %>%
  group_by(year) %>%
  summarise(
    sum_race = sum(raceword),
    year_total = n()
  )
```
